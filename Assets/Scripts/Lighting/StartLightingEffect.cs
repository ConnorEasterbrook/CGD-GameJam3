...
public class StartLightingEffect : MonoBehaviour
{
    ...
    void Update()
    {
        _incrementalSkyboxColor = Color.Lerp(_incrementalSkyboxColor, _skyboxMaterial.color, (Time.deltaTime / 10));
        _skyboxMaterial.color = _incrementalSkyboxColor;

        _incrementalAmbientGroundColor = Color.Lerp(_incrementalAmbientGroundColor, _ambientGroundColor, (Time.deltaTime));
        RenderSettings.ambientLight = _incrementalAmbientGroundColor;

        _incrementalLightingIntensity = Mathf.Lerp(_incrementalLightingIntensity, _lightingIntensity, (Time.deltaTime / 10));
        _lightingObject.GetComponent<Light>().intensity = _incrementalLightingIntensity;

        _incrementalFogAmount = Mathf.Lerp(_incrementalFogAmount, _maxFogAmount, (Time.deltaTime / 10));
